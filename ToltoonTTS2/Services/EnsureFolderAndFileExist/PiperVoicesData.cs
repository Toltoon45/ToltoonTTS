...
namespace ToltoonTTS2.Services.EnsureFolderAndFileExist
{
    public static class PiperVoicesData
    {
        ...
        //это все голоса, которые можно скачать. Не трогай
        ...
        public static async Task EnsureFileExists()
        {
            var lockTaken = false;
            try
            {
                ...
                bool hasPiperExe = File.Exists(Path.Combine(piperEngine, "piper.exe"));
                bool hasAnyPiperBinary = Directory.EnumerateFiles(piperEngine, "piper*").Any();
                if (!hasPiperExe && !hasAnyPiperBinary)
                {
